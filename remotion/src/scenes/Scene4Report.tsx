...
export const Scene4Report: React.FC = () => {
  ...
  const score = Math.round(interpolate(frame, [25, 70], [0, 80], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }));
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.bg, padding: "120px 60px 80px" }}>
      <div style={{ fontFamily: FF_KO, fontSize: 38, color: COLORS.subtext, opacity: titleS }}>오늘의 리포트</div>
      <div style={{ fontFamily: FF_KO, fontSize: 64, color: COLORS.text, marginTop: 8, opacity: titleS, letterSpacing: -2 }}>마음 점수</div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 16, marginTop: 16 }}>
          <div style={{ fontFamily: FF_LATIN, fontSize: 220, fontWeight: 700, lineHeight: 1, letterSpacing: -8 }}>{score}</div>
          <div style={{ fontFamily: FF_LATIN, fontSize: 60, opacity: 0.8 }}>/ 100</div>
        </div>
        <div style={{ fontFamily: FF_KO, fontSize: 32, marginTop: 24, opacity: 0.95 }}>오늘 하루 잘 보내셨네요</div>
      </div>
      ...
      <div style={{
        ...
        <Img src={staticFile("images/fortune-cookie-closed.png")} style={{ width: 180, height: 180, objectFit: "contain" }} />
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: FF_KO, fontSize: 32, color: "#A07A1F" }}>오늘의 포춘쿠키</div>
          <div style={{ fontFamily: FF_KO, fontSize: 44, color: COLORS.text, marginTop: 8, letterSpacing: -1 }}>탭해서 열어보세요 ›</div>
        </div>
      </div>
    </AbsoluteFill>
  );
